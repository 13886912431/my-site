...
// 随机头像
for (let i = 1; i <= 8; i++) {
    avatar.push(require(`@/assets/img/avatar/avatar${i}.jpg`))
}
...
Mock.mock(/\/setting/, "get", {
    ...
    data: {
        ...
        siteTitle: "刘胤麟的博客",
        github: "https://github.com/13886912431", 
        qq: "619030878", 
        qqQrcode: require('@/assets/img/qq-qrcode.png'),
        weixin: "LYL619030878", 
        weixinQrcode: require("@/assets/img/wx-qrcode.png"),
        mail: "619030878@qq.com", 
        icp: "© 备案号",
        githubName: "Serein.",
    }    
})
            ...
            [`rows|${query.limit}`]: [
                {
                    ...
                    title: "@ctitle",
                    desc: "@cparagraph",
                    thumb: "@image('500x500', @color)", // 缩略图
                    createdAt: Date.now(),
                    "commentCount|1-40": 30,
                    "scanCount|1-10": 10
                }
            ],
            // rows: []
...
// 文章分类
Mock.mock(/\/article\/classify/, "get", ({ url }) => {
    console.log(url);
    return Mock.mock({
        code: 0,
        msg: "@cword(4)",
...
Mock.mock(/\/article\/.+/, "get", ({ url }) => {
    ...
    return Mock.mock({
        ...
        data: {
            ...
            "scanCount|0-10000": 0, //浏览次数
            "commentCount|0-10000": 0, //评论数
            desc: "博客描述，显示到列表页",
            createdAt: Date.now(), // 时间戳，创建日期
            //博客章节目录
            toc: [
            ...
            thumb: "@image('250x200')" // 缩略图
        }
    });
});

// 分页获取评论
Mock.mock(/\/comment(\?.+)?$/, "get", ({ url }) => {
    console.log(url);
    const query = parseQuery(url);
    return Mock.mock({
        code: 0,
